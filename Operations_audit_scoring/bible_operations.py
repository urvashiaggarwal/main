...
class ScoreCalculator:
    ...
    def calculate_score(self, row, data_point_name):
        # Calculate score for Bible matching (primary vs secondary sources)
        ref_val = row['ref_normalised']
        primary_val = row['primary_normalised']
        secondary_val = row['secondary_normalised']
        
        # Determine matching method based on data point name
        ...
        elif 'Project Price range' in data_point_name:
        ...
        if self.normalize_value(ref_val) is None:
            ...
            # For Bible matching: score is 1 if ref matches either primary or secondary, else 0
            score = 0
            
            # Check match with primary source
            if self.normalize_value(primary_val) is not None:
                if match_method == 'exact_string':
                    if self.exact_string_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'numeric_gte':
                    if self.numeric_gte_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'numeric_exact':
                    if self.numeric_exact_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'project_area_match':
                    if self.project_area_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'avg_price_psft_match':
                    if self.avg_price_psft_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'project_price_range_match':
                    if self.project_price_range_match(ref_val, primary_val):
                        score = 1
                elif match_method == 'rera_number_match':
                    if self.rera_number_match(ref_val, primary_val):
                        score = 1
            
            # Check match with secondary source only if no match with primary
            if score == 0 and self.normalize_value(secondary_val) is not None:
                if match_method == 'exact_string':
                    if self.exact_string_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'numeric_gte':
                    if self.numeric_gte_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'numeric_exact':
                    if self.numeric_exact_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'project_area_match':
                    if self.project_area_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'avg_price_psft_match':
                    if self.avg_price_psft_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'project_price_range_match':
                    if self.project_price_range_match(ref_val, secondary_val):
                        score = 1
                elif match_method == 'rera_number_match':
                    if self.rera_number_match(ref_val, secondary_val):
                        score = 1
...
class MySQLProcessor:
    ...
    def update_normalization_values(self, results: List[DataPointScore]):
        for r in results:
            update_query = """
                UPDATE bible_op_data
                SET ref_normalised = %s, primary_normalised = %s, secondary_normalised = %s, is_scored = 1
                WHERE `index` = %s AND data_point_name = %s
            """
            self.cursor.execute(update_query, (
                r.ref_normalised,
                r.primary_normalised,
                r.secondary_normalised,
                r.index,
                r.data_point_name
            ))
            self.conn.commit()
    ...
    def process_normalization(self, data_point, instruction):
        ...
        if not rows:
            print(f"No unscored rows found for normalization: {data_point}")
            return
        df = pd.DataFrame(rows)
        results = self.client.get_norm_values(df, data_point, instruction)
        self.update_normalization_values(results)
        print(f"Normalized {len(results)} rows for {data_point}")
    ...
    def process_scoring(self, data_point):
        ...
        for row in rows:
            try:
                ...
                print(f"Error calculating score for row {row.get('index')}: {e}")
